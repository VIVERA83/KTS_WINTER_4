...
from bot.vk.keyboards.data_classes import Round, TimeoutKeyboard
...
class AnswerKeyboard(Keyboard):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.keyboard = KeyboardSchema(name="AnswerKeyboard", buttons=base_structure, one_time=False)
        self.settings: Optional["Round"] = Round()
        self.timeout_keyboard = TimeoutKeyboard(keyboard=RoundResultKeyboard,
    ...
    def create_buttons(self):
        ...
        for key, user_id in enumerate(self.settings.users, 2):
            if f"{self.settings.answers[f'{user_id}a'].value}" != "Нет ответа":
                buttons[1] = deepcopy(base_structure[1])
                button_answer = Button(
                    name=f"{user_id}a",
                    label=f"{self.settings.answers[f'{user_id}a'].value}",
                    color=TypeColor.blue,
                )
                # вычитываем кол-во голосов за данный ответ
                sum_vote = sum(self.settings.votes[f'{user_id}v'].value.values())
                ...
                ic(self.settings.votes, sum(self.settings.votes[f'{user_id}v'].value.values()))
    ...
    async def button_vote(self, message: MessageFromVK) -> KeyboardEventEnum:
        ...
        message.event_data = self.keyboard.get_help_string_from_title(message.payload.button_name)
    ...
    async def button_answer(self, message: MessageFromVK) -> KeyboardEventEnum:
        ...
        from bot.vk.keyboards.round_result import RoundResultKeyboard
        if user_data := self.settings.users.get(f"{message.user_id}"):
            if user_data.value:
                message.body = f"{self.get_user(message.user_id).name} : Ваш ответ {message.body}"
                if message.body == self.settings.correct_answer:
                    self.settings.experts += 1
                    self.settings.watcher -= 1
                return await self.redirect(RoundResultKeyboard, deepcopy(self.users), is_private=True,
